...
const runProctoring = async () => {
  if (videoRef.current && isInterviewStarted && !showResult && videoRef.current.readyState === 4) {
    try {
      // Small optimization: Using TinyFaceDetector with lower input size for speed
      ...
      if (!detection) {
        // Warning sirf tab badhao jab face bilkul na dikhe
        ...
        if (Math.abs(nosePos - faceCenter) > eyeDist * 0.6) {
          setProctoringMessage("DON'T LOOK AWAY");
          setWarnings(prev => prev + 1);
        } else {
          // Sab sahi hai toh message clear kar do
          setProctoringMessage("");
        }
      }

      // Termination logic: 150 points (approx 15-20 seconds of violation)
      if (warnings > 3) {
        alert("INTERVIEW TERMINATED: Proctoring violation limits exceeded.");
        window.location.reload();
      }
    } catch (err) {
  ...
  const nextQuestion = async () => {
    ...
    setIsListening(false); 
    const currentAns = transcript || answers[currentQuestionIndex] || "User provided no answer";
    ...
    try {
      ...
      const incomingQuestion = res.data.nextQuestion || res.data;
      const codingSignal = res.data.isCodingRound;
      const codingKeywords = ["write", "code", "program", "function", "script", "algorithm", "develop", "logic", "implement", "coding"];
      const containsCodeKeyword = codingKeywords.some(word => incomingQuestion?.toLowerCase().includes(word));
      setIsCodingRound(!!(codingSignal || containsCodeKeyword)); 
      setQuestions(prev => [...prev, incomingQuestion]);
      setCurrentQuestionIndex(prev => prev + 1);
    } catch (err) {
      alert("Backend slow hai...");
      setIsCodingRound(false); 
    } finally {
